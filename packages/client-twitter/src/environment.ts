...
export const twitterEnvSchema = z.object({
    ...
    // Make these optional if cookies are provided
    TWITTER_PASSWORD: z.string().optional(),
    TWITTER_EMAIL: z.string().optional()
  }).refine((data) => {
    // Require either cookies OR (password AND email)
    return (!!data.TWITTER_COOKIES) || (!!data.TWITTER_PASSWORD && !!data.TWITTER_EMAIL);
  }, {
    message: "Either TWITTER_COOKIES or both TWITTER_PASSWORD and TWITTER_EMAIL must be provided"
  });
